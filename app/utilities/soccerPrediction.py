...
class SoccerGoalPredictor:
    def __init__(self, friendly_league_name, year=None):
        ...
        self.predictions_filepath = self.generate_filepath('current_predictions')
    ...
    def generate_filepath(self, file_type):
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'season_stats', self.league_long_name))
        ...
        elif file_type == 'current_predictions':
            return os.path.join(base_dir, f'current_predictions_{self.league_long_name}.csv')
        else:
            raise ValueError("Invalid file type. Use 'cleaned_data', 'recent_stats', 'model_home', 'model_away', 'scaler', or 'current_predictions'.")
        ...
        for home_team in teams:
            for away_team in teams:
                if home_team != away_team:
                    home_goals, away_goals = self.predict_goals(home_team, away_team)
                    predictions.append({
                        'home_team': home_team,
                        'away_team': away_team,
                        'home_goals_pred': home_goals,
                        'away_goals_pred': away_goals
                    })

        predictions_df = pd.DataFrame(predictions)
        predictions_df.to_csv(self.predictions_filepath, index=False)
        print(f"Predictions saved to {self.predictions_filepath}")
...
def generate_predictions_for_league(friendly_league_name):
    predictor = SoccerGoalPredictor(friendly_league_name)
    predictor.generate_all_predictions()

if __name__ == '__main__':
    generate_predictions_for_league('MLS')
